...
function __asyncjs__bash_web(argc,argv,out_buf,out_size) { return Asyncify.handleAsync(async () => { const args = []; for (let i = 0; i < argc; i++) { args.push(UTF8ToString(HEAP32[(argv >> 2) + i])); } const result = await globalThis.__bash_web_internal(args); if (typeof result === 'string') { stringToUTF8(result, out_buf, out_size); return 0; } return result; }); }
